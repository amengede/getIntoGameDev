...
class ObjModel:
    ...
    def draw(self, position, outline, outlineColor, shader = None):
        if outline:
            #original model
            ...
            model_transform = pyrr.matrix44.multiply(model_transform,pyrr.matrix44.create_from_scale(scale=np.array([1.1,1.1,1.1]),dtype=np.float32))
            model_transform = pyrr.matrix44.multiply(model_transform,pyrr.matrix44.create_from_translation(position, dtype=np.float32))
            glUseProgram(shader)
            glUniformMatrix4fv(glGetUniformLocation(shader, "model"),1,GL_FALSE,model_transform)
            glUniform3fv(glGetUniformLocation(shader, "color"), 1, outlineColor)
            glBindVertexArray(self.vao)
            glDrawArrays(GL_TRIANGLES,0,self.vertexCount)
            glStencilMask(255)
            glStencilFunc(GL_ALWAYS, 1, 255)
