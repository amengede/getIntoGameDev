...
class GameApp:
    ...
    def handleKeys(self):
        ...
        if keys[pg.K_w]:
            self.scene.move_player(direction = 0, amount = 0.0025*self.frameTime)
        elif keys[pg.K_a]:
            self.scene.move_player(direction = 90, amount = 0.0025*self.frameTime)
        elif keys[pg.K_s]:
            self.scene.move_player(direction = 180, amount = 0.0025*self.frameTime)
        elif keys[pg.K_d]:
            self.scene.move_player(direction = -90, amount = 0.0025*self.frameTime)
