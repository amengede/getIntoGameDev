...
class App:
    def __init__(self):
        ...
        self.imageAvailableSemaphore = None
        self.renderFinishedSemaphore = None
    ...
    def createSemaphores(self):
        semaphoreInfo = VkSemaphoreCreateInfo(
            sType=VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO
        )

        self.imageAvailableSemaphore = vkCreateSemaphore(self.device, semaphoreInfo, None)
        self.renderFinishedSemaphore = vkCreateSemaphore(self.device, semaphoreInfo, None)
    ...
    def drawFrame(self):
        ...
        imageIndex = vkAcquireNextImageKHR(self.device, self.swapchain, 18446744073709551615, #uint64 max: disables timeout
                                           self.imageAvailableSemaphore, VK_NULL_HANDLE) #trigger imageAvailableSemaphore when image has been retrieved

        submitInfo = VkSubmitInfo(
            sType=VK_STRUCTURE_TYPE_SUBMIT_INFO,
            waitSemaphoreCount = 1,
            pWaitSemaphores = [self.imageAvailableSemaphore,], #wait on imageAvailableSemaphore before starting command queue (drawing)
            ...
            pSignalSemaphores = [self.renderFinishedSemaphore,] #semaphore to trigger upon queue completion
            )
        ...
        presentInfo = VkPresentInfoKHR(
            ...
            waitSemaphoreCount=1,
            pWaitSemaphores=[self.renderFinishedSemaphore,], #presentation waits until draw queue has finished
            swapchainCount=1,
        ...
        vkQueueWaitIdle(self.presentQueue)
    ...
    def exit(self):
        ...
        vkDestroySemaphore(self.device, self.imageAvailableSemaphore, None)
        vkDestroySemaphore(self.device, self.renderFinishedSemaphore, None)
