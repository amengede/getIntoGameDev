use glfw::Window;
use crate::{model::transform_component::TransformComponent, renderer::backend::{
    bind_group_layout, definitions::{SkeletalModel, SkeletalVertex}, mesh_builder::{self, ObjLoader}, pipeline, texture::{new_color, new_depth_texture, new_texture, Texture}, ubo::{UBOGroup, UBO}}};
...
impl<'a> State<'a> {
    ...
    fn update_transforms(&mut self, quads: &Vec<Object>, tris: &Vec<Object>,
                        characters: &Vec<TransformComponent>) {
        ...
        for i in 0..characters.len() {
            let matrix = characters[i].get_transform();
            self.ubo.as_mut().unwrap().upload(offset + i as u64, &matrix, &self.queue);
        }
    }
