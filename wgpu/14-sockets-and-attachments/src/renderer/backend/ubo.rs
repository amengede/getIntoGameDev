use crate::renderer::backend::mesh_builder::vec_to_u8_slice;
...
impl UBO {
    ...
    pub fn new(device: &wgpu::Device, layout: &wgpu::BindGroupLayout, byte_size: u64, label: &str) -> Self {
    ...
    pub fn upload_vec(&mut self, matrices: &Vec<glm::Mat4>, queue: &wgpu::Queue) {
        let offset = 0;
        let data: &[u8] = unsafe { vec_to_u8_slice(matrices) };
        queue.write_buffer(&self.buffer, offset, data);
    }
}