...
class Game:
    ...
    def __init__(self):
        ...
        self.renderer = Renderer(self.screen_width, self.screen_height, 
                                 self.map, self.player, self.player_state)
        ...
        while (running):
            ...
            for event in pg.event.get():
                ...
                if (event.type == pg.MOUSEBUTTONDOWN
                    and self.can_shoot
                    and self.player_state[1] > 0):
                    self.can_shoot = False
                    self.player_state[1] = max(0, self.player_state[1] - 1)
            ...
            self.renderer.update(self.frametime)
    ...
    def handle_keys(self) -> None:
        ...
        if magnitude == 0:
            publish(MESSAGE_TYPE_PLAYER_STOP, self.observers)
            return
        publish(MESSAGE_TYPE_PLAYER_WALKING, self.observers)
