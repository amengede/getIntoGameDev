...
def game_mode(screen_surface: pg.Surface, screen_pixels: np.ndarray) -> int:
    ...
    RED = map_rgb((128, 0, 0), shifts)
    BLUE = map_rgb((0, 0, 128), shifts)
    ...
    while running:
        ...
        if ball[0] < 50:
            paddle_lives[0] -= 1
            if paddle_lives[0] == 0:
                title = make_text(font, "Player 2 wins", BLUE)
                blit(title, screen_pixels, (200, 100))
                wait_for_key()
                return GAME_MODE_PLAY
            else:
                place_ball(ball, ball_speed)
        
        if ball[0] > 750:
            paddle_lives[1] -= 1
            if paddle_lives[1] == 0:
                title = make_text(font, "Player 1 wins", RED)
                blit(title, screen_pixels, (200, 100))
                wait_for_key()
                return GAME_MODE_PLAY
            else:
        ...
        for i in range(paddle_lives[0]):
            draw_rect(screen_pixels, [100 + 15 * i, 30, 10, 20], RED)
        for i in range(paddle_lives[1]):
            draw_rect(screen_pixels, [700 - 15 * i, 30, 10, 20], BLUE)
...
def place_ball(ball: list[int], speed: list[int]) -> None:
    ball[0] = 400
    ball[1] = 300
    ball_speed = 12
    while True:
        theta = np.random.random() * 2 * np.pi
        speed[0] = int(ball_speed * np.cos(theta))
        speed[1] = int(ball_speed * np.sin(theta))
        if speed[0] != 0:
            break
