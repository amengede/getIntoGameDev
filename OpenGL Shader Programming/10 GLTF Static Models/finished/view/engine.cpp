...
void Engine::createModels() {
	...
	cubeModel2 = new GLTFStaticMesh("models/cube.gltf", 0.4f);
...
void Engine::render(Scene* scene) {
	...
	woodMaterial->use();
	float scale = cubeModel2->prepareForDrawing();
	glUniformMatrix4fv(model.standard, 1, GL_FALSE,
		glm::value_ptr(glm::scale(scene->cube->modelTransform, glm::vec3(scale)))
	);
	glUniformSubroutinesuiv(GL_FRAGMENT_SHADER, 1, &lightingFunction.rough);
	cubeModel2->draw();

	glUniformMatrix4fv(model.standard, 1, GL_FALSE,
		glm::value_ptr(glm::scale(scene->cube2->modelTransform, glm::vec3(scale)))
	);
