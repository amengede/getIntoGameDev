...
vk::ShaderEXT make_compute_shader(vk::Device logicalDevice,
    const char* name,
    ...
    */
    ...
    preprocess_shader(info);
    compile_file_to_assembly(info);
    std::vector<uint32_t> srcCode = compile_file(info);
    ...
    shaderInfo.setCodeSize(sizeof(uint32_t) * srcCode.size());
    shaderInfo.setPCode(srcCode.data());
    shaderInfo.setPName(pName);
    shaderInfo.setSetLayoutCount(1);
    shaderInfo.setPSetLayouts(pLayouts);
    ...
    */
    vk::ShaderEXT shader;
    auto result = logicalDevice.createShadersEXT(1, &shaderInfo, nullptr, &shader, dl);
    VkShaderEXT handle = shader;


    if (result == vk::Result::eSuccess) {
        logger->print("Successfully made compute shader");
        deletionQueue.push_back([logger, handle, dl](vk::Device device) {
