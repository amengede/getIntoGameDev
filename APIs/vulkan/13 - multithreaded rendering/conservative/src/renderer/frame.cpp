...
Frame::Frame(vk::DispatchLoaderDynamic& dl, 
	...
	vk::Queue& queue):
	logicalDevice(logicalDevice), dl(dl), swapchain(swapchain), shaders(shaders),
	queue(queue) {
	...
	//Color attachment: prebaked stuff
...
void Frame::acquire_image_and_record_draw_commands() {
	...
	imageIndex = logicalDevice.acquireNextImageKHR(
		swapchain.chain, 1e9, imageAquiredSemaphore, nullptr).value;
