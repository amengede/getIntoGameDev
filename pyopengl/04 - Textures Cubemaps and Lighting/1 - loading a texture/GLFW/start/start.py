...
class Material:
    ...
    def __init__(self, filepath):
        ...
        """
            Task: configure the sampler

            Hint: use glTexParameteri
        """
        #TODO: set the wrap mode to repeat for s and t coordinates
        #TODO: set the minifying filter to nearest mipmap linear 
        # (choose the appropriate mipmap level, then linearly interpolate it)
        #TODO: set the magnifying filter to linear
        with Image.open(filepath, mode = "r") as image:
            image_width,image_height = image.size
            image = image.convert("RGBA")
            img_data = bytes(image.tobytes())
            """
            Task: send the loaded image data to the graphics card.

            tell openGL to take a texture image 2D,
                the target is a 2D texture, send it to mipmap level 0
                store it in an internal format of rgba,
                set width and height appropriately,
                use border color 0,
                pixel format coming in is rgba,
                data type is unsigned byte
                for the data pointer, simply pass in img_data
            """
        glGenerateMipmap(GL_TEXTURE_2D)

    def use(self):
        """
            Task: bind the texture at the appropriate 
            texture unit.
        """
        pass
        #TODO: set texture unit 0 as the active texture
