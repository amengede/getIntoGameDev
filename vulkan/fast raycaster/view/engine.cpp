...
void Engine::make_pipelines() {
	...
	computePipelineBuilder.specify_compute_shader("shaders/raycaster.spv");
	computePipelineBuilder.add_descriptor_set_layout(frameSetLayout);

	vkInit::ComputePipelineOutBundle computeOutput = computePipelineBuilder.build();

	pipelineLayout = computeOutput.layout;
	pipeline = computeOutput.pipeline;
...
void Engine::prepare_frame(uint32_t imageIndex, Scene* scene) {
	...
	_frame.wallBuffer.blit(
		(void*)scene->walls.data(), scene->walls.size() * sizeof(Wall));
	_frame.cameraBuffer.blit(
		(void*)&scene->description, sizeof(SceneDescription));

	_frame.write_descriptor_set();
}

void Engine::prepare_to_raycast_barrier(vk::CommandBuffer commandBuffer, vk::Image image) {
...
void Engine::dispatch_raycast(vk::CommandBuffer commandBuffer, uint32_t imageIndex) {

	commandBuffer.bindPipeline(vk::PipelineBindPoint::eCompute, pipeline);

	commandBuffer.bindDescriptorSets(vk::PipelineBindPoint::eCompute, pipelineLayout, 0, swapchainFrames[imageIndex].descriptorSet, nullptr);

	commandBuffer.dispatch(static_cast<uint32_t>(swapchainExtent.width / 64), 1, 1);
...
void Engine::render(Scene* scene) {
	...
	prepare_to_raycast_barrier(commandBuffer, swapchainFrames[imageIndex].image);
	dispatch_raycast(commandBuffer, imageIndex);
