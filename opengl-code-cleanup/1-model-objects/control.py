...
class GameApp:
    ...
    def create_objects(self):
        ...
        self.scene = model.Scene(self.shaders, monkey_model, ground_model, skyBoxModel, self.wood_texture)
    ...
    def mainLoop(self):
        ...
        self.handleKeys()
        #update objects
        self.scene.update([self.shader3DColored, self.shader3DTextured, self.shader3DBillboard, self.shader3DCubemap, self.shader3DOutline])
    ...
    def lightRender(self):
        player_pos = self.scene.player.transform_component.position
        skybox = self.scene.skyBox
        cube = self.scene.cube
        light = self.scene.light
        light2 = self.scene.light2
        monkey = self.scene.monkey
        ground = self.scene.ground

        glEnable(GL_DEPTH_TEST)
        glDisable(GL_CULL_FACE)
        skybox.draw(player_pos)
        glEnable(GL_CULL_FACE)
        cube.draw(self.shader3DLightMap)
        light.draw(self.shader3DLightMap)
        light2.draw(self.shader3DLightMap)
        monkey.draw(False, None, self.shader3DLightMap)
        ground.draw(self.shader3DLightMap)
    ...
    def renderScene(self):
        ...
        light2.draw(None)
        ground.draw(None)
        monkey.draw(True, np.array([0,1,0], dtype=np.float32), self.shader3DOutline)
        self.smoke.draw(np.array([-1,0.5,0.5],dtype=np.float32), player_pos)

    def handleKeys(self):
        keys = pg.key.get_pressed()
        if keys[pg.K_w]:
            self.scene.move_player(0, 0.0025*self.frameTime)
            return
        if keys[pg.K_a]:
            self.scene.move_player(90, 0.0025*self.frameTime)
            return
        if keys[pg.K_s]:
            self.scene.move_player(180, 0.0025*self.frameTime)
            return
        if keys[pg.K_d]:
            self.scene.move_player(-90, 0.0025*self.frameTime)
            return
